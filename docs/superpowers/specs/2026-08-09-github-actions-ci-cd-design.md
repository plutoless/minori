...
The active `main` ruleset is source-controlled at `.github/rulesets/main.json`. It requires pull requests and the three observed CI check contexts `CI / verify`, `CI / integration`, and `CI / image-amd64`. Its only bypass actor is GitHub User `plutoless` (actor ID `471561`) with `pull_request` bypass mode, for the Emergency Merge Bypass when broken CI governance would otherwise prevent its own repair. It is not a normal merge path, does not allow direct pushes or direct production deployment, and does not bypass the release workflow's tag, ancestry, validation, or Production Approval gates.

The active `v*` tag ruleset is source-controlled at `.github/rulesets/release-tags.json`; it prevents update and deletion with an empty bypass list. An incorrectly created release tag remains immutable; the operator corrects the version and creates a new tag rather than moving the old one. Creating a new protected release tag remains an explicit operator action. A release workflow still independently verifies package-version equality and `main` ancestry; repository rules are not treated as the only enforcement layer.
