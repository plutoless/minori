...
Historical content may be cited, but Minori does not present it as a live result from the current run. Claims about the current or latest state, permissions, versions, or read failures use evidence actually obtained in that run or say they have not been verified live. Cache and `as of` qualifiers remain visible, and member-provided facts remain attributed until a tool verifies them. This is an instruction boundary with no intent classifier or mandatory tool sequence.

The Processing Reaction remains Minori's immediate durable acknowledgement. If a supported member-triggered request is still processing after 20 seconds, Minori may send one fixed ordinary **Progress Reply**: `我还在处理这条请求，完成后会继续回复。` It is best-effort, never periodic, and not appended to Retained Conversation History; Feishu may later return it as powerless Live Group History. `Typing` remains until the existing terminal cleanup. Scheduled Runs do not send Progress Replies.

Normal Agent answers, authenticated Sources, operation receipts, and Scheduled Task results are **Rich Content Reply** messages sent by the Feishu SDK as Feishu `post` content with one `md` element. Markdown images become ordinary links and are never fetched or uploaded automatically. Progress and fixed failure notices are **Control Reply** messages sent as plain SDK text. Lark CLI remains strict-user-only and is never used for messaging. This rendering change does not alter prompts, Team Context, conversation history, retry policy, or answer length.
...
Before enabling meeting evidence, publish an app version with these additive user read scopes, then rerun `npm run lark:auth` for the Dedicated Knowledge User: `contact:user:search`, `vc:meeting.search:read`, `vc:meeting.meetingevent:read`, `vc:record:readonly`, `vc:note:read`, `minutes:minutes.search:read`, `minutes:minutes.basic:read`, and `minutes:minutes.transcript:export`. `vc:record:readonly` lets Minori open a selected Meeting Record after lightweight discovery. The advanced `vc:meeting.artifact.verbatim:read` scope is not part of the default OAuth set in this tenant; an unavailable original-transcript source degrades independently and does not block Meeting Record search, readable summaries, or Minute transcripts. Because all delivered members can receive content visible to that user, review the Dedicated Knowledge User's native access before publishing the scopes. Do not add Calendar, contact-directory export, meeting/media write, permission, sharing, or upload scopes. Keep Lark CLI at the release-locked 1.0.84 for this contract fix; upgrade it separately.
...
- `SCHEDULE_DEFAULT_TIMEZONE` (default `Asia/Shanghai`), `SCHEDULE_ENABLED`, `SCHEDULE_POLL_MS` (default `15000`), and `SCHEDULE_LEASE_MS` (default `420000`, covering the Agent timeout plus terminal delivery)
...
1. Send a private message containing a heading, bullets, emphasis, a link, and a code block; verify a Rich Content Reply without visible Markdown markers, `Typing` after durable persistence, and terminal `Typing` removal.
...
9. Re-run one real knowledge read and verify its Source is clickable, then disposable create, append, and exact patch operations. Preserve the Write Replay Boundary evidence, and manually remove the disposable document in Feishu because Minori has no delete tool.
10. Enable the scheduler only after disabled preflight. Create one private one-time task and one uniquely targeted group recurrence; verify each result is rich top-level content with clickable Sources, no Typing, no topic reply, current Team Context, optional Scheduled Context cutoff, one latest-only catch-up after restart, and no automatic retry after a controlled terminal failure.
